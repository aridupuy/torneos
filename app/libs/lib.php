...
{
    developer_log("DATA_BASE_".$databaseType);
    developer_log("DATA_BASE_".$type);
    developer_log("DATA_BASE_".$errorNo);
    developer_log("DATA_BASE_".$errorMsg);
    developer_log("DATA_BASE_".$sql);
    developer_log("DATA_BASE_".json_encode($inputarr));
...
function calcular_digito1_cvu($cbu){
//  $cbu=trim($cbu);
    if(strlen($cbu)!=7)
            return false;
    $cbu="$cbu";
...
function calcular_digito2_cvu($cbu){
    $cbu="$cbu";
    $cuenta = 0;
    $pontificador=array(3,9,7,1,3,9,7,1,3,9,7,1,3);
    //0001234100000000894045 ejemplo
    for ($i = 8; $i < 21; $i++) {
...
function cast2object($instance,$class){
...
function object2array(Object $object){
    ...
    return json_decode(str_replace("\u0000$class\u0000","",json_encode((array)$object)), true);
}
...
function calcular_password($password_text){
    $rs = Model::execute_select("SELECT 
    SHA1(?) as pass", [$password_text]);
    return $rs->fetchRow()["pass"];
}
...
/*esta funcion saca las millas que hay entre dos pares de latitudes y longitudes */

function distanciaMillas($lat1=0,$lon1=0,$lat2=0,$lon2=0){
    $aux1= [floatval($lat1),floatval($lon1)];
    $aux2= [floatval($lat2),floatval($lon2)];
    $finishDistance = distanciaKM($aux1,$aux2);
    return $finishDistance*0.621371;
}
function distanciaKM($aux1,$aux2){
    $RadioTierraKm = hexdec("6378.0F");
    
    $difLat = enradianes($aux1[0]-$aux2[0]);
    $diflon = enradianes($aux1[1]-$aux2[1]);
    $a = pow(sin($difLat/2),2)
            + cos(enradianes($aux1[0]))
            * cos(enradianes($aux2[0]))
            * pow(sin($diflon/2),2);
